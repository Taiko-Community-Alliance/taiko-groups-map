...
export const initialize = (points, element, options = {}) => {
	// Create map
	const map = L.map(element, {zoomControl: false}).setView([37.7749, L.Util.wrapNum(-122.4194, [0,360], true)], 3); 
	L.control.zoom({
		position: 'topright'
	}).addTo(map);
	...
	const mapMarkerLayer = initializeTaikoMap(map, rxPoints, options);
